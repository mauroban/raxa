...
- **Corrigir nível tem dois modos** (na ficha, só admin — D-73). **A partir de agora** (padrão): o nível atual vira exatamente o degrau escolhido — o `base` (rating de entrada) absorve a diferença, e o que as partidas já renderam fica dentro do ajuste; é a correção de quem olhou a escada e discordou do hoje. **Desde a entrada**: o degrau escolhido vira o próprio `base` — é o palpite que faltou no cadastro — e todo o histórico **reaplica por cima** (`rebuildAll`): o racha que já aconteceu passa a contar a partir do nível certo. O modo fica no registro de correções. Nos dois casos, só `base` vai para o banco; Elo e patente são recalculados dele + histórico.
