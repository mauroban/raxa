...
**Na tela a divisão é um risco, não um número (D-80).** O badge mostra o nome da patente e **1, 2 ou 3 traços inclinados** — `OURO /`, `OURO //`, `OURO ///`. Mais riscos, mais alto: a contagem é lida de longe, no chip do time e na escada, sem depender só da cor (que diferencia a patente, não a divisão). O acabamento continua o de sempre — divisão 1 fosca, 2 metal, 3 polida com brilho (D-79) — e agora os riscos também o carregam. O número existe em texto corrido ("agora é Ouro 3", toasts, registro de correções) e no `title`/`aria-label` de todo badge, que é o que o leitor de tela anuncia. É o mesmo desenho em **todo lugar** em que a patente aparece: badges longos e curtos, o pontinho dos chips de time e dos slots, a escada da aba Jogadores (só os riscos, porque o cabeçalho já diz a patente), a ficha, o editor de nível do admin, os cards de subida/queda do fim do racha e a variação de nível dos destaques.
