...
Ordem de grandeza no racha curto, assentado (K=16, times parelhos): mover uma **divisão** exige cerca de **8 vitórias líquidas** (vitórias menos derrotas, já contando a margem) — dois rachas bons; uma **patente** inteira, cerca de 25. Ao entrar sem palpite (K=64) é a quarta parte disso; com palpite (K=32), a metade — e o K desce conforme a pessoa joga (D-82).
...
O rating numérico é quase invisível: na aba Jogadores, **só o admin** vê o Elo cru de cada um, num número pequeno e apagado no canto da linha (com a dica "Elo — só o admin vê"). Ao lado dele, quando os resultados de alguém vêm **persistentemente contra o que o nível prevê** (10+ partidas na valência e média móvel de resultado − esperado fora de ±0,35, ~2 desvios do ruído), o admin vê **⚠ rende acima/abaixo do nível — revisar palpite?** na escada e na ficha (D-82). É aviso para uma correção humana — o motor nunca mexe sozinho por isso: em simulação, K automático por sequência ou por surpresa amplifica ruído, mas como alerta ele ataca exatamente o caso que os dados não corrigem (palpite errado de quem joga pouco). Para todo o resto — e para todos os outros papéis — o número não existe: a linguagem pública é a patente.
