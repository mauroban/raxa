...
**Período (D-149):** `Último` (racha), `Mês`, `Ano` ou `Sempre`. Os três primeiros abrem no **atual** (último racha, mês de hoje, ano de hoje) e ganham uma linha **‹ setembro de 2026 ›** logo abaixo do filtro, que anda para o racha, o mês ou o ano anterior; o botão passa a mostrar o escolhido (12/09, ago/26, 2025) e, tocado de novo, volta ao atual. Cada período traz a história quebrada **no passo abaixo dele**: no mês, **racha a racha**; no ano, **mês a mês**; em Sempre, **ano a ano** — com gráfico de colunas (aproveitamento; partidas) e uma linha por grupo (rachas, partidas, gols, tempo, aproveitamento). É a resposta direta para "quantos rachas joguei esse ano" e "e na vida?".
