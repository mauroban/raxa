...
| **Código da Liga** | código curto de 6 caracteres (`RXA7Q2`), visível nos ajustes; quem digita **pede para entrar** e o admin aprova em Membros — **implementado** | alguém que ouviu falar do racha e quer entrar. O admin manda o **link de convite** (`…/?c=CODIGO`, botão Enviar/Copiar link em Ajustes): quem abre cria a conta ou entra e o pedido vai sozinho, sem digitar nada (D-132). A tela abre em *Criar conta* para quem nunca entrou neste aparelho, mas só na primeira vez: tocar em *Entrar* vale e fica (D-209). O admin troca o código em Ajustes → **Novo** quando ele vazou: o antigo deixa de valer na hora e quem já entrou não muda nada (D-130). |
...
A regra única, nos três: **ninguém entra sem aceitar, e ninguém entra sem o admin querer.** Hoje só o código existe, e ele **gera pedido, não entrada** — o admin aprova em Jogadores → Pendências, numa folha que já diz **quem é** ("Ele escolhe", "+ Novo: @conta" ou um perfil sem dono, os de nome parecido primeiro) e **o cargo** (Lançador marcado); o botão diz o resultado — "Aprovar · Juliano Rocha" — e um toque aprova, vincula e dá o cargo (D-217). O link com vencimento, a entrada livre e o convite direto de uso único são desenho da v2.
...
| Papel | Pode |
|---|---|
| **Admin** | Tudo: configurar a liga (os ajustes são só dele — os outros papéis veem o formato e a aparência, sem controles, D-128), revisar/corrigir/anular partidas, cadastrar e **arquivar** jogadores (apagar de vez só quem nunca jogou — D-128), juntar cadastros, dar papéis (rebaixar a Jogador é o que tira o peso das opiniões de alguém — D-121), ver na ficha tudo que um membro opinou sobre os outros (D-124) |
| **Moderador** | O que o lançador faz; revisar e corrigir partida (gols, escalação, trocas), anular e apagar são só do admin (D-198) |
| **Lançador** | **Padrão de cadastro novo** (D-216). Conduzir o racha: presença, times, partidas, gols, cadastrar jogador, **editar nome, descrição e hábito de gol** de qualquer jogador (D-128) — e **opinar sobre o nível** de entrada de qualquer um (como moderador e admin) |
| **Jogador** | Vê ranking, histórico e as próprias estatísticas; vincula o próprio perfil; pode contestar; pode dar opinião sobre nível, mas ela **só passa a valer** quando for Lançador ou acima (D-121). Não lança nada |
...
Todo jogador cadastrado nasce **Lançador** (D-216): no racha quem estiver com o celular lança. **Jogador** — só olha, e a opinião não vale — é a exceção que o admin escolhe. Os perfis que já existiam mantêm o cargo que tinham. Conta ainda sem perfil vinculado é tratada como Jogador até dizer quem é.
...
**Reativar** fica na própria ficha (faixa "arquivado em …") e os arquivados aparecem num card dos ajustes do admin. **Apagar cadastro** só existe para quem **nunca jogou uma partida** — cadastro por engano; a presença que houver em algum racha sai junto. Para quem já jogou, o botão nem aparece, e a ação recusa.

**A ficha, de cima para baixo:** cabeçalho com o **nome editável no próprio nome** e, embaixo, uma **descrição** discreta (apelido, "amigo do Matheus", como chegou — até 120 caracteres; quem lança edita, os outros leem) → nível (cartões Linha/Gol, entrada, e quantas de cada patente só para moderador ou acima, D-197) → números → as linhas de **costuma ir ao gol** (quem lança), **conta** e **permissão** (só admin) → bloco **Admin** (opiniões que deu, juntar cadastros, arquivar, apagar). **Não há Salvar** (D-215): interruptor, permissão e conta gravam no toque; nome e descrição ao sair do campo. Pergunta antes só deixar de ser admin você mesmo e desvincular a conta de outra pessoa. Logo abaixo dos números, **Ver números de X ›** abre a aba Stats já na pessoa. Quem pode o quê: **nome, descrição e hábito de gol, quem lança** (lançador, moderador, admin); **qualquer outra alteração** — permissão, arquivar, reativar, apagar — **só o admin**; a conta, o próprio dono ("Sou eu" só aparece para quem ainda não tem perfil nesta liga; desvincular, o dono ou o admin).

**Edição que não se perde (D-176).** Se você renomeia, muda a descrição ou dá uma opinião e, antes de a gravação subir, outra pessoa grava algo no mesmo jogador (uma opinião, um vínculo), os dois valem: o app mescla campo a campo, e a opinião é por autor. Ninguém perde o apelido por ter editado ao mesmo tempo.
