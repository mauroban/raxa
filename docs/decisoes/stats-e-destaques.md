...
<a id="d-208"></a>
### D-208 · Leão do fim do racha: gols quando quem está em quadra já tem 50+ min nas pernas
**Quando:** 2026-09-21 (três versões no mesmo dia — abaixo).
...
partidas (`fim`). Um ranking novo nos gerais de Mês, Ano e Sempre, logo depois de Gols por hora:
**Leão do fim do racha** — gols de linha por hora de linha no fim, com 20 min de linha lá e o
piso de presença de todo ranking. No racha aberto (período Último), o card Destaques do racha
ganha o Leão daquele racha: gols de linha nas partidas do fim dele, em total como o artilheiro da
noite (a dica diz quantas partidas foram; desempate por menos tempo de linha lá). Na ficha, "fim
do racha" entra na faixa Posição nos rankings, só na leitura de linha.
...
o cansaço médio é o que se desconta do rating, um dia; o individual é leitura). A primeira versão
usava o **último terço das partidas** de cada racha; o dono pediu: "considere isso quando as pessoas
em campo começarem o racha com mais de 50 min de jogo naquele dia em média, não no último terço" —
cansaço é tempo nas pernas, não posição na lista: num racha curto ninguém está cansado, e num longo
o fim começa antes do último terço. A média é de quem começa a partida, porque é o cansaço daquele
confronto. A segunda versão trazia dois rankings, o de resultado (acima do esperado no fim) na
frente; o dono cortou: "você focou em resultados, vamos focar em gols". Ficou um ranking só, de
gols, na mesma régua dos outros ritmos (taxa por hora de linha, piso de presença, patente no
desempate).
**Descartado:** último terço das partidas (proporção, não cansaço); contar por relógio de parede
(pausa e espera não cansam); só o tempo de quem está lendo, sem média (a partida é cansada ou não
para todo mundo em quadra); ranking de resultado no fim (acima do esperado / aproveitamento ali —
a segunda versão; o pedido é gol); gols totais no período (D-145: tudo é taxa); mexer no K ou no
rating (D-83, D-195: só com ~100 partidas, e nunca por pessoa).
**Onde:** `FIM_MIN`, `fimDoRacha`, campo `fim` em `statsLiga`, `leao` em `listasRk`, rankings
`leao`/`rleao` e a faixa de posições em `viewStats`, ícone `hourglass` em `SICO`, tudo em
`index.html` · `scripts/smoke.py` (12 partidas de 10 min, todo mundo sempre em quadra: da 7ª em
diante cada um tem 60 min nas pernas; 6 gols, só nas 6 do fim; ranking nos gerais e no racha
aberto; posição na ficha) · [Stats](../produto/stats.md).
