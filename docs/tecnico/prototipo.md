...
**Revisão mostra a partida inteira; a correção reescreve o log de eventos.** A revisão (admin) remonta a partida a partir dos trechos e do log de eventos: ficha de quem jogou (tempo em quadra, papel, gols, entrou/saiu), linha do tempo com gols, substituições, trocas de goleiro e pausas, os trechos com escalação e peso, e o efeito no nível — por papel, quando a pessoa fez linha e gol na mesma partida (D-118). "Corrigir partida" (ou um toque num gol) abre o rascunho: gols e trocas são eventos do log, e editar, adicionar ou apagar um gol reescreve o log; `recalcPartida` reconta `goals`, `score` e `result` dos eventos e refaz os trechos (D-198). Não existe resultado lançado à parte do placar.

**Substituição, vaga e "foi embora".** No racha curto, trocar alguém durante a partida vale para o lado (D-129): quem saiu vai para o fim da fila, quem entrou é do lado e guarda a preferência (`lv.pref`, a posição que tinha na fila) — se o lado perder e ele tiver que sair, `rodaFila` o devolve a esse lugar; se ficar, a preferência acaba no fim da partida. O `↶` desfaz lado e fila (evento `sub` com `roda`). Na partida única a única troca que fica é titular ⇄ reserva do mesmo time. Lado com gente a menos é **completado na hora pela frente da fila** (D-129) — sem empréstimo; sem ninguém na fila, a vaga tracejada fica à vista ("＋ vaga · entra o próximo") e um toque nela põe o próximo, ou o nome tocado antes. Quem se machuca ou precisa ir embora sai por **🚑 Foi embora** (botão na pré-partida): sai da presença, do time, da fila e do rodízio; se estava em quadra, a partida segue com um a menos e o trecho seguinte registra isso. Quem ainda não jogou hoje pode sair como **marcado por engano**, sem contar presença.

**Escalação sempre à vista.** Na pré-partida e na partida ao vivo, logo abaixo do nome/placar de cada time, aparece a escalação em duas colunas: o goleiro num slot próprio em cima (🧤, o ponto do nível pela patente de goleiro, e ↻ se vier do rodízio) e um slot por vaga de linha (5 no 5v5; vaga vazia fica tracejada). Quem entrou nesta partida leva a seta verde ▲ e quem saiu, na fila, a vermelha ▼ (o ⇄ de substituto saiu, D-129). Ao vivo, tocar num nome substitui (ou arraste um reserva/alguém de fora sobre ele); o slot 🧤 e a vaga de linha entram na mesma gramática (de fora para o 🧤 = entra no gol e o goleiro sai; de linha para o 🧤 do mesmo lado = trocam de papel; 🧤 com 🧤 = trocam de lado; de linha para o 🧤 vazio = vai ao gol e a vaga da linha fica marcada), sem folha — D-117. Quem descansa no rodízio aparece na fila, como todo mundo. Na pré-partida a coluna tem o nome do time em cima e as reservas embaixo, e é ali que se edita, **com a mesma gramática da partida ao vivo** (D-123): nome ⇄ nome troca de lugar de vez; de fora → 🧤 põe no gol (só nesta partida com rodízio; com goleiro fixo entra no time no lugar do goleiro); de linha ⇄ 🧤 trocam de papel nesta partida; de fora → vaga entra de vez; 🧤 ⇄ 🧤 trocam de gol; nome em quadra → fila (arraste, ou toque no nome e no espaço da fila) vai para o fim e a frente da fila entra; dois nomes da fila trocam de ordem (D-129). No card da fila o chip carrega a chave `fora:ID` — é a pessoa, não o slot do gol que ela ocupa. Toda mexida na mão tem ↶ desfazer (`preHist`). A folha antiga do goleiro continua por "outras opções" na dica, com o 🧤 marcado. A tela em repouso não explica nada: a dica só aparece com um nome marcado.

**Goleiro da próxima partida.** Goleiro é do time só quando há um para cada time da montagem (D-140); aí ele sai e entra com o time, e um goleiro que chega espera na fila com o 🧤 e entra no gol de quem perder (D-129). Fora disso é rodízio (`gkPool`): com dois ou mais, cada um fica no seu gol e o que sobra espera e entra no gol de quem perder; com um só, **o goleiro fica com o lado que fica em quadra** — o que venceu ou, no empate, o que ficou; no resto, troca de lado. A pré-partida mostra o goleiro no slot 🧤 de cada lado e qualquer um pode ser trocado por toque/arraste — por outro do rodízio (grupo "🧤 Rodízio" no card Fora), por alguém do time ou por alguém da fila — antes do apito; com rodízio a escolha vale só para aquele confronto (`nextGks`). "Foi embora" fica na pré-partida (ao lado de "Chegou agora"), e não na substituição. Não há botão de "girar a fila": troca fora do automático é toque/arraste entre fila e time.
...
**A roda (D-129).** Na montagem `lv.teams` são os times inteiros que cabem (2 a 4, `planTeams().grupos`, contados pela linha); `startJogo`/`startMatch` chamam `fechaMontagem`: os dois primeiros ficam como lados, os outros viram `lv.fila` na ordem, e a montagem inteira fica em `lv.montagem` (`timesMontados` é o que a sessão grava, D-50). Daí em diante `lv.teams` tem sempre dois lados e `lv.fila` é a fila de pessoas, na ordem. Ao fim da partida (`finish` → `rodaFila`): quem ganhou fica; do lado que perdeu sai quem está há mais tempo em quadra (`seguidasHoje`, partidas seguidas nas escalações; desempate `jogosHoje`), tantos quantos a fila de linha puder repor; entra a frente da fila; quem saiu vai para o fim; o goleiro fixo só roda se há goleiro na fila. Empate: a fila repõe os dois lados → os dois rodam; repõe um → roda o que está há mais tempo em quadra; senão ninguém. `completaLados` fecha um lado curto pela frente da fila (contando quem vai ao gol com rodízio), chamado no fim da partida, na largada, no "foi embora" e no "sai para a fila". `lv.ult` guarda o resumo (placar, quem saiu, quem entrou, quem ficou) para o cartão fixo da próxima partida, com o ↩ Voltar a partida; `lv.preHist` são as fotos para o ↶ desfazer da pré-partida. Racha gravado antes com 3–4 times é convertido ao carregar (times que esperavam → fila).
...
- Goleiros: fixos no time só quando há um por time; senão rodízio — um em cada gol, o que espera entra no gol de quem perder, um só reveza de lado; improviso no meio da partida
...
- **Leão do fim do racha** (D-208): gols por hora de linha no fim (partidas em que quem começa já tem, em média, 50+ min de jogo no racha) menos no resto do racha — nos gerais, no racha aberto e na faixa de posições da ficha
...
- **Confirmação de presença pelo app** (D-165): agenda semanal da liga, lista abre N dias antes a uma hora escolhida (D-184), "Vou"/"Vou no gol" com espera por papel derivada da hora do servidor, quem lança confirma pelos outros (e a pessoa assume com "Confirmo", D-178), texto para o grupo, cancelar a data; no dia, a presença já vem marcada. E **"Quem é você nesta liga?"** (D-166): o membro novo escolhe o próprio perfil ou cria o seu com apelido
