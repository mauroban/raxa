...
console.log('\n[15] dupla inseparavel');
{
  ...
  for(let i=0;i<25;i++)lanca([x.id,y.id,...outros.slice(0,3)],outros.slice(3,8),i%2,{sid:'s-cola'+Math.floor(i/8)});
  const j=inseparaveis(liga);
  ok(j.some(d=>(d.a===x.id&&d.b===y.id)||(d.a===y.id&&d.b===x.id)),'25 partidas sempre juntos: a dupla e apontada');
